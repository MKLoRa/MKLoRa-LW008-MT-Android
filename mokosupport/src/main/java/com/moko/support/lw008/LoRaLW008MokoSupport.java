...
import com.moko.support.lw008.handler.MokoCharacteristicHandler;
...
public class LoRaLW008MokoSupport extends MokoBleLib {
    private HashMap<OrderCHAR, BluetoothGattCharacteristic> mCharacteristicMap;

    private static volatile LoRaLW008MokoSupport INSTANCE;

    private Context mContext;

    private MokoBleConfig mBleConfig;

    private LoRaLW008MokoSupport() {
        //no instance
    }

    public static LoRaLW008MokoSupport getInstance() {
        if (INSTANCE == null) {
            synchronized (LoRaLW008MokoSupport.class) {
                if (INSTANCE == null) {
                    INSTANCE = new LoRaLW008MokoSupport();
                }
            }
        }
        return INSTANCE;
